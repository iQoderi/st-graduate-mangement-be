...
function editMyMsg(req, res, next) {
    ...
    if (req.user.role === '学生') {
        for (var item in req.user.teacher) {
            if (!updateData[item]) {updateData[item] = req.user.students[item]}
        }
        var update = {$set: {students: updateData}};
    } else {
        for (var item2 in req.user.students) {
            if (!updateData[item]) {updateData[item] = req.user.teacher[item]}
        }
    ...
    Users.update(condition, update).exec((err, info)=> {
		...
		if(req.user.role==='学生'){
			...
			Graduate.update(condition,updateBys).exec((err,info)=>{});
                ...
                res.json({code: 10000, data: {msg: '修改个人信息成功'}})
            } else {
                res.json({code: 10011, data: {msg: "用户未找到"}})
            }
        }
    });
}
